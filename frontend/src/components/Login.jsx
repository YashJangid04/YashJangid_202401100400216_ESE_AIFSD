...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('https://yashjangid-202401100400216-ese.onrender.com/api/auth/login', formData);
        ...
        <div className="bg-white py-8 px-4 shadow-xl shadow-slate-200/40 sm:rounded-3xl sm:px-10 border border-slate-100">
          ...
          <div className="mt-8">
            ...
            <div className="mt-6 text-center">
              <Link to="/register" className="text-indigo-600 hover:text-indigo-500 font-bold text-sm">
                Create an account
              </Link>
            </div>
          </div>
